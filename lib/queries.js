    ...
    try {
      ...
      itemData = await db
        ...
        .aggregate([
          ...
          { $project: { "description": { $arrayElemAt: [{ $split: ["$description", "|"] }, position] }, name: 1, mysqlId: 1, planId: 1, image: 1, price: 1, location: 1 } },
          {
            $facet: {
              items: [
              ...
              quantityLocation: [
                {
                  $group: {
                    _id: { location: { $arrayElemAt: [{ $split: ["$location", "|"] }, 0] } },
                    quantity: { $sum: 1 }
                  }
                }
              ],
